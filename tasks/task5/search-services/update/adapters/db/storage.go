...
func (db *DB) Add(ctx context.Context, comics core.Comics) error {
	_, err := db.conn.Exec(`INSERT INTO comics (comics_id, comics_url, words) VALUES ($1, $2, $3);`, comics.ID, comics.URL, comics.Words)
...
func (db *DB) Stats(ctx context.Context) (core.DBStats, error) {
	rows, err := db.conn.QueryContext(ctx, `SELECT words FROM comics;`)
...
func (db *DB) IDs(ctx context.Context) ([]int, error) {
	rows, err := db.conn.QueryContext(ctx, `SELECT comics_id FROM comics;`)
